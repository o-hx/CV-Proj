...
if __name__ == '__main__':
    ...
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    autoencoder = torch.load(os.path.join(os.getcwd(),'weights','flower_Autoencodercurrent_model.pth'), map_location = device)
    ...
    # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    # autoencoder = torch.load(os.path.join(os.getcwd(),'weights','flower_clustering_model.pth'), map_location = device)
    ...
    plot_clusters(model = autoencoder,
                ...
                batch_size = batch_size,
                original_dataloader = org_validation_dataloader,
